...
type Params = Promise<{ category_id: string; component_id: string }>

export default async function Page({ params }: { params: Params }) {
  const { category_id, component_id } = await params

  const categories = getEnrichedComponentCategories()
  const result = findComponentItem(category_id, component_id)
  ...
  const { category, item: component } = result
  const sampleCode = component.source

  const currentIndex = category.items.findIndex((i) => i.id === component.id)
  ...
  const previous = previousItem
    ? {
        ...
        href: `/components/${category_id}/${previousItem.id}`,
      }
    : undefined
  const next = nextItem
    ? {
        id: nextItem.id,
        name: nextItem.name,
        href: `/components/${category_id}/${nextItem.id}`,
      }
    : undefined
        ...
        <div className="flex flex-col gap-10">
          <Breadcrumb className="font-mono text-xs">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbLink render={<Link href={`/components/${category.id}`} />}>
                  {category.name.toLowerCase()}
                </BreadcrumbLink>
              </BreadcrumbItem>
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>{component.name.toLowerCase()}</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
